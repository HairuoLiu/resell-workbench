...
**把家里的闲置东西，一次性挂到 6 个二手平台。**

拍张照 → 让 AI 帮你写好文案 → 你看一眼确认 → 发布到 Facebook、Kijiji、小红书、Karrot、闲鱼、Carousell。
...
- 同一件桌子，要在 6 个平台各填一遍表单 😩
- 英文文案写得干巴巴，发出去没人问
- 发了一堆东西，过两周完全想不起来"这个椅子到底在哪个平台挂过"
- 某个平台其实发失败了，你压根不知道

**这个工作台就是来解决这些的。** 一件东西录一次，六个平台的文案自动生成好，
每个平台发成功还是失败一目了然。
...
| 会不会很难，我不会用？ | 界面就三步：录入 → 待确认 → 发布。跟着下面的图走就行 |
...
## 🆓 想完全免费、照片不出门？用本机 Ollama

不想把家里照片传给任何第三方、或者干脆没网？可以用**本机模型** Ollama：

- 照片**不出这台电脑**，不花一分钱，断网也能用
- 不需要 API Key
...
> ollama serve
> ```
> 不设这个，浏览器会被跨域拦住、连不上（报 "Failed to fetch"）。Mac / Linux 同理设 `OLLAMA_ORIGINS=*` 再启动。
> 模型要自己拉：`ollama pull qwen2.5vl`（识图）、`ollama pull qwen2.5`（翻译）。
...
![待确认页面](docs/screenshots/02-review.png)
...
如果你启动了「自动发布助手」（见下面的进阶章节），
勾选想发的平台，点一下 **「发布选中平台」**，
它会自动打开浏览器，**自己填表、自己上传照片、自己点发布**。

你就在旁边看着就行。

> 没启动自动发布助手时，这个按钮是**灰色的**，会显示「自动发布助手离线」——
...
**Q：右上角圆点一直是灰的 / 红的？**
A：说明自动发布助手没启动。如果你不用全自动功能，**这完全不影响**，忽略它就行。
...
└── automation/bridge/      ← 可选：真·一键发布的本机自动发布助手服务
